...
if [  -f "/var/www/${APP_NAME}/composer.json" ]; then
    if [ ! -f "/var/www/${APP_NAME}/CONFIGURA_TALLSTACKUI" ]; then
        cd /var/www/${APP_NAME}
...
echo "/var/www/${APP_NAME}/tailwind.config.js original:"
cat /var/www/${APP_NAME}/tailwind.config.js
echo "-------------------------"
echo ""
echo "adicionando paths..."
...
in_content && /\],/ {
    print "        '\''./app/**/*.php'\'', // <- Livewire + Controllers";
    print "        '\''./resources/js/**/*.js'\'', // <- JS, se usar";
    print "        '\''./resources/js/**/*.vue'\''";
    in_content=0
}
{ print }
' /var/www/${APP_NAME}/tailwind.config.js > /var/www/${APP_NAME}/tailwind.config.js.tmp && mv /var/www/${APP_NAME}/tailwind.config.js.tmp /var/www/${APP_NAME}/tailwind.config.js

echo ""
echo "/var/www/${APP_NAME}/tailwind.config.js alterado:"
cat /var/www/${APP_NAME}/tailwind.config.js
echo "-------------------------"
...
<body>
    ...
    <x-layout>
        <x-slot:header>
            <x-layout.header without-mobile-button class="bg-[#152f4e] max-h-24">
                <div class="max-h-24">
                <x-slot:left>
                    <img src="{{ asset('pics/Logo_DGTi+Uerj_fundoEscuro+transparencia-01.png') }}"  class="max-h-24 max-w object-contain block">
                </x-slot:left>
                <x-slot:middle>
...
EOF
        ...
        touch /var/www/${APP_NAME}/CONFIGURA_TALLSTACKUI
    else 
